...
export interface TimeSuspicionResult {
  suspicious: boolean;
  reason: string;
}
...
export class PlagiarismService {
  ...
  /**
   * Detect whether code was likely AI-generated using LLM analysis.
   */
  async detectAIGenerated(code: string, language: string): Promise<AIDetectionResult> {
    try {
      if (!this.apiKey) {
        this.logger.debug('No LLM API key configured, skipping AI detection');
        return { isAI: false, confidence: 0, reasons: [] };
      }

      const systemPrompt = `You are an AI code detection expert. Analyze the following code and determine if it was likely written by a human or generated by AI (like ChatGPT, Claude, Copilot). Return JSON only.

Signs of AI-generated code:
- Overly consistent formatting throughout
- Generic variable names like temp, result, data, output
- Excessive or perfectly written comments
- Boilerplate patterns and textbook-perfect implementations
- Suspiciously well-structured error handling
- Overly verbose or defensive code

Signs of human-written code:
- Inconsistent formatting or style
- Personal naming conventions
- Minimal or no comments
- Shortcuts or quick hacks
- Debug prints or TODO comments left in
- Imperfect but functional implementations

Return ONLY valid JSON in this exact format:
{"isAI": true, "confidence": 0.85, "reasons": ["reason1", "reason2"]}
...
confidence should be between 0.0 and 1.0.`;
      ...
      if (!response.ok) {
        ...
        this.logger.warn(`AI detection LLM API error ${response.status}: ${error}`);
        return { isAI: false, confidence: 0, reasons: [] };
      }
      ...
      if (!content) {
        this.logger.warn('Empty response from AI detection LLM');
        return { isAI: false, confidence: 0, reasons: [] };
      }
      ...
      this.logger.error(`AI detection failed: ${error}`);
      return { isAI: false, confidence: 0, reasons: [] };
    }
  }

  /**
   * Check if submission timing is suspicious given the difficulty and code length.
   */
  checkTimeSuspicion(
    ...
    // Always suspicious: 500+ chars in under 60 seconds
    if (codeLength > 500 && timeSpentSeconds < 60) {
      return {
        suspicious: true,
        reason: `Code is ${codeLength} characters but was completed in only ${timeSpentSeconds}s (under 60s threshold for long submissions)`,
      };
    }

    const expectedMinimum = expectedMinimums[difficulty] || 120;

    if (timeSpentSeconds < expectedMinimum && codeLength > 100) {
      return {
        suspicious: true,
        reason: `Completed in ${timeSpentSeconds}s, expected at least ${expectedMinimum}s for ${difficulty} difficulty with ${codeLength} characters of code`,
      };
    }
